...
登录后，顶部导航按当前身份列出全部功能。当前滚动章节以白色高亮；点击导航项会直接打开该功能的第一个详情操作。也可以继续滚动首页镜头，或使用右侧操作卡进入具体业务。详情页地址格式为 `/workspace/{role}/{feature}/{action}`，页内可切换同一功能的其他操作。顶部主题切换支持跟随系统、浅色和深色。
